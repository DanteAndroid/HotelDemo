...
fun OrdersScreen(navController: NavController, vm: OrdersViewModel = viewModel()) {
    ...
    val resetFlow = entry?.savedStateHandle?.getStateFlow("orders_reset", false)
    val shouldReset by (resetFlow?.collectAsStateWithLifecycle() ?: remember { mutableStateOf(false) })
    LaunchedEffect(shouldReset) {
        if (shouldReset) {
            entry?.savedStateHandle?.set("orders_reset", false)
            vm.reset()
        }
    }
